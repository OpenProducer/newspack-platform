...
- **2026-08-28**: Closed the long-standing "GitHub sync for `sync-plugins.sh`" open question (found while running `sync-plugins.sh --env podcast` for real and noticing the commit never showed up on `github`). `sync-plugins.sh`'s updates land via `terminus env:commit` directly on Pantheon's internal git — the local checkout never saw them, unlike `sync-themes.sh` which commits locally and could already push to both remotes. Added a final step to `sync-plugins.sh` (both repos) that checks out the mapped local branch, fetches `origin`, refuses to proceed if the local checkout is dirty or has unpushed local commits on that branch, fast-forward-merges the Pantheon commit in, pushes to `github`, then restores whatever branch was checked out before. Any failure WARNs with manual recovery steps rather than forcing anything — the Pantheon commit itself was already safe either way. `bash -n` passes on both scripts.
- **2026-08-14**: Automattic archived every dedicated Newspack GitHub repo and merged them into `Automattic/newspack-workspace`, silently breaking both `sync-plugins.sh`'s theme-release check and `newspack-plugin-update-checker` (a third-party plugin covering 6 plugins never on WordPress.org). Deactivated `newspack-plugin-update-checker` everywhere; widened `sync-plugins.sh`'s existing GitHub-release mechanism to cover those 6 plugins from the new monorepo instead. Found and fixed a real pagination bug during rollout (prerelease tags were satisfying the "found" check before stable releases were fetched). Rolled out for real across every environment on both `newspack-platform` and `newspack-radio-pro`, each verified individually. Full detail in "GitHub-release workspace packages" below.
...
#### GitHub-release workspace packages — plugins + themes (added 2026-07-25, widened 2026-08-14)
...
**2026-08-14 update — Automattic archived the per-plugin GitHub repos.** On 2026-08-06, Automattic archived every dedicated Newspack GitHub repo (`newspack-plugin`, `newspack-ads`, `newspack-blocks`, `newspack-popups`, `newspack-listings`, `newspack-sponsors`, `newspack-theme`, `newspack-block-theme`, and others) and merged them into one monorepo, `Automattic/newspack-workspace`. This broke two things at once, silently (no errors — each just froze on its last pre-archival release forever):

- This script's own theme-release check (which pointed directly at `Automattic/newspack-theme` and `Automattic/newspack-block-theme`'s dedicated `releases/latest` endpoints).
- `wp-content/plugins/newspack-plugin-update-checker` (a small third-party plugin, not from Automattic, using the YahnisElsts PluginUpdateChecker library) — hardcoded to `https://github.com/Automattic/{slug}` for six plugins that were never on WordPress.org: `newspack-plugin`, `newspack-ads`, `newspack-blocks`, `newspack-popups`, `newspack-listings`, `newspack-sponsors`. This was the only thing making `wp plugin update --all` see updates for those six at all.

**Decision: deactivate `newspack-plugin-update-checker` rather than patch it, and widen `sync-plugins.sh`'s existing GitHub-release mechanism to cover the six plugins too, instead of writing a new script or a subclass.** The vendored PluginUpdateChecker library derives its version number straight from the tag name before any custom release filter runs, and the monorepo's shared tags are `{slug}@{version}` — there's no clean hook to strip that prefix before the library uses it for version comparison. Patching it properly means subclassing library internals; extending logic that already lives in this script and already does tag-based GitHub-release polling was the simpler, more visible fix. `newspack-plugin-update-checker`'s plugin files are left in place, just deactivated on every environment — re-enable it instead of this section only if it's ever updated upstream for monorepo-style tags.

How the mechanism works now:

- One config array, `GH_WORKSPACE_PACKAGES`, replaces the old `GH_THEME_REPOS`: rows of `tag_prefix|type|wp_slug|zip_asset_name`, one row per zip asset (`newspack-theme`'s single release still covers 6 zips — base + 5 named style variants — so it appears 6 times, once per zip/slug). Covers `newspack` (the core plugin — confirmed its monorepo tag prefix changed from `newspack-plugin` to `newspack`, but its release zip is still named `newspack-plugin.zip`; prefix and zip name differ for this one entry only), `newspack-ads`, `newspack-blocks`, `newspack-popups`, `newspack-listings`, `newspack-sponsors`, `newspack-theme` (+5 variants), `newspack-block-theme`.
- All of it is fetched from one shared feed — `GET /repos/Automattic/newspack-workspace/releases`, paginated (up to 5 pages of 100) — instead of each package having its own `releases/latest`. The feed spans roughly 20 unrelated packages with heavy pre-release noise (`-alpha.N`, `-hotfix-*`), so matching is done by tag PREFIX, trusting GitHub's own `prerelease`/`draft` flags rather than reimplementing semver comparison locally — same philosophy as before, just applied to a shared feed instead of a dedicated one.
- **Real bug hit and fixed during rollout (2026-08-14):** the pagination early-stop check originally counted a tag prefix as "found" the moment ANY tag with that prefix appeared — including prereleases. A site-wide hotfix wave put a prerelease tag for every tracked package on page 1 simultaneously, satisfying the check for everyone and stopping pagination before the real stable releases (living on later pages) were ever fetched. Confirmed via a live dry-run: 7 of 8 packages came back "no stable release found" until this was fixed. Fix: the early-stop check now only counts a prefix as found once a release that is both non-`draft` and non-`prerelease` has actually appeared, matching the per-package resolution logic exactly. Verified afterward via independent re-fetch of all release pages, computing true latest-stable-by-publish-date per package, and confirming the script's resolved versions matched exactly (8/8 on this repo, same fix mirrored on newspack-radio-pro's `sync-plugins.sh`).
- Per slug: compares `terminus wp $SITE_ENV -- <plugin|theme> get <slug> --field=version` against the resolved release's version. If not installed on that environment, skips it with a note rather than force-installing something the site never had.
- If different: downloads the matching `<zip_asset_name>` release asset and `wp <plugin|theme> install <url> --force`s it in the same SFTP session as the wp.org updates, folded into the same commit.
- Bash 3.2 compatible by construction (flat array + `IFS='|' read`, not `declare -A`) — this repo's stock macOS bash bit us once already on `sync-themes.sh`, not repeating that mistake here.
- No new dependency: plain `curl` against `api.github.com` (public repo, unauthenticated rate limit of 60/hr is ample for a handful of paginated calls per run), no `gh` CLI requirement.
...
Rolled out for real on 2026-08-14 across every environment on both `newspack-platform` and `newspack-radio-pro` (dev→test→live promotion plus direct runs on `radio`/`podcast`/`develop`/`playerv1`), each verified individually (site load, correct versions, guarded plugins/themes untouched) before moving to the next. `newspack-plugin-update-checker` confirmed deactivated on every environment on both sites beforehand.
...
~~2. GitHub sync for `sync-plugins.sh`~~ — resolved 2026-08-28: `sync-plugins.sh` (both `newspack-platform` and `newspack-radio-pro`) now pulls the Pantheon-side commit into the matching local branch and pushes it to `github` as its final step, mirroring `sync-themes.sh`'s existing origin+github push. Best-effort by design — WARNs and prints manual recovery steps rather than forcing a checkout/merge if the local checkout is dirty, diverged, or has unpushed local commits on the target branch; the Pantheon commit itself is never at risk. Env→branch mapping: `dev`→`master`, `radio`→`radio`, `podcast`→`podcast` on this repo; `dev`→`master`, `develop`→`develop`, `playerv1`→`playerv1` on `newspack-radio-pro`.
