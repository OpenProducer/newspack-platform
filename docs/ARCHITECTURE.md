...
- **Remaining for full rollout**: visual confirmation of podcast's new carousel behavior; Studio symlink setup (open question #3, still unresolved — no Studio site exists for this repo on any branch); owner visual checks on `test`/`live` after today's promotion (in progress); real `sync-plugins.sh` runs on `radio` and `podcast` (on hold pending the `live` visual check).
