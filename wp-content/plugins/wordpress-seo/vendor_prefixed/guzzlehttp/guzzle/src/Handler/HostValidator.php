...
final class HostValidator
{
     ...
     */
    public static function assertRequestHost(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request) : void
    {
     ...
     * @throws RequestException
     */
    private static function assertUriHostValue(string $value, \YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request) : void
    {
        if (!self::isPrintableAscii($value)) {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException(\sprintf('The request URI host "%s" must contain only printable ASCII characters, because a handler can otherwise connect to a host that differs from the one the request names. An internationalized host name has an A-label form that this rule accepts.', self::escape($value)), $request);
        }
        if (\strpos($value, '%') !== \false) {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException(\sprintf('The request URI host "%s" must not contain a percent escape, because a handler can decode it and then connect to a host that differs from the one the request names.', self::escape($value)), $request);
        }
    }
    /**
     ...
     * @throws RequestException
     */
    private static function assertHostHeaderValue(string $value, \YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request) : void
    {
        if (!self::isPrintableAscii($value)) {
        ...
        if (\strpos($value, '%') !== \false) {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException(\sprintf('The request Host header "%s" must not contain a percent escape, because an intermediary or an origin server can decode it and then read it as an authority that differs from the one the request names.', self::escape($value)), $request);
        }
    }
    /**
     ...
     * @throws RequestException
     */
    private static function assertNoAuthorityDelimiter(string $host, \YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request) : void
    {
        ...
        if (\preg_match('/\\A[^\\/?#@\\\\]*\\z/D', $host) !== 1) {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException(\sprintf($message, self::escape($host)), $request);
        }
     ...
     * @throws RequestException
     */
    private static function assertNotADottedAddress(string $host, \YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request) : void
    {
        if (\substr($host, -1) !== '.') {
            return;
        }
