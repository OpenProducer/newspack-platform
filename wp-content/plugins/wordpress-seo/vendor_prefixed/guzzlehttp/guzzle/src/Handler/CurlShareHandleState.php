...
final class CurlShareHandleState
{
    ...
    public static function fromOption($sharing) : ?self
    {
        ...
        if ($mode === \YoastSEO_Vendor\GuzzleHttp\TransportSharing::NONE) {
            return null;
        }
        if ($mode === \YoastSEO_Vendor\GuzzleHttp\TransportSharing::HANDLER_PREFER) {
    ...
    public static function normalizeMode($sharing, string $option) : string
    {
        ...
        if ($sharing === null || $sharing === \YoastSEO_Vendor\GuzzleHttp\TransportSharing::NONE) {
            return \YoastSEO_Vendor\GuzzleHttp\TransportSharing::NONE;
        }
        if ($sharing === \YoastSEO_Vendor\GuzzleHttp\TransportSharing::HANDLER_PREFER || $sharing === \YoastSEO_Vendor\GuzzleHttp\TransportSharing::HANDLER_REQUIRE) {
            return $sharing;
        }
    ...
    public static function assertNoRequiredSharingCustomFactoryConflict(array $options, string $handlerName) : void
    {
        ...
        if ($mode !== \YoastSEO_Vendor\GuzzleHttp\TransportSharing::HANDLER_REQUIRE) {
            return;
        }
    ...
    {
        \YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::ensureHandlerSharingSupported();
        if ($mode === \YoastSEO_Vendor\GuzzleHttp\TransportSharing::HANDLER_REQUIRE) {
            \YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::ensureSslSessionSharingSupported();
        }
        $locks = [self::requireCurlConstant('CURL_LOCK_DATA_DNS')];
        if (\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsSslSessionSharing()) {
