...
class CurlFactory implements \YoastSEO_Vendor\GuzzleHttp\Handler\CurlFactoryInterface
{
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \YoastSEO_Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \YoastSEO_Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    ...
    private function applyHandlerOptions(\YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy, array &$conf) : void
    {
        ...
        if (isset($options['proxy'])) {
            if (!\is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    if (isset($options['proxy']['no']) && \YoastSEO_Vendor\GuzzleHttp\Utils::isUriInNoProxy($easy->request->getUri(), $options['proxy']['no'])) {
                        $proxyConf = '';
                        $noProxyConf = '*';
                    } else {
        ...
        if ($proxyConf === null) {
            $proxyConf = \YoastSEO_Vendor\GuzzleHttp\Handler\ProxyEnvironment::getProxyForScheme($easy->request->getUri()->getScheme());
            if ($proxyConf === null) {
                $proxyConf = '';
            } elseif (($noProxy = \YoastSEO_Vendor\GuzzleHttp\Handler\ProxyEnvironment::getNoProxy()) !== null && \YoastSEO_Vendor\GuzzleHttp\Utils::isUriInNoProxy($easy->request->getUri(), \YoastSEO_Vendor\GuzzleHttp\Handler\ProxyEnvironment::splitNoProxy($noProxy))) {
        ...
        if (\is_string($proxyConf) && $proxyConf !== '') {
            ...
            if ($scheme !== null && \preg_match('/^[a-z][a-z0-9.+-]*$/D', $scheme) !== 1) {
                ...
                // wire.
                throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException('The proxy URL is malformed.', $easy->request);
            }
            if ($scheme === 'https' && !\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsHttpsProxy()) {
                ...
                throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException('HTTPS proxies are not supported by the installed libcurl; libcurl 7.52.0 or newer built with HTTPS-proxy support is required.', $easy->request);
            }
        }
