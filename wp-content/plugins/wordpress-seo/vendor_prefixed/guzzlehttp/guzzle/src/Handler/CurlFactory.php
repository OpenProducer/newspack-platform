...
class CurlFactory implements \YoastSEO_Vendor\GuzzleHttp\Handler\CurlFactoryInterface
{
     ...
     */
    public function __construct(int $maxHandles, string $shareMode = \YoastSEO_Vendor\GuzzleHttp\TransportSharing::NONE, $shareHandle = null)
    {
        $this->maxHandles = $maxHandles;
        $this->shareMode = \YoastSEO_Vendor\GuzzleHttp\Handler\CurlShareHandleState::normalizeMode($shareMode, 'transport_sharing');
        if ($shareHandle instanceof \YoastSEO_Vendor\GuzzleHttp\Handler\CurlShareHandleState) {
        ...
        if ($this->shareMode === \YoastSEO_Vendor\GuzzleHttp\TransportSharing::NONE && $shareHandle !== null) {
    ...
    public function create(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array $options) : \YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle
    {
        ...
        $requiredMultiplex = \in_array($multiplex, [\YoastSEO_Vendor\GuzzleHttp\Multiplexing::REQUIRE_EAGER, \YoastSEO_Vendor\GuzzleHttp\Multiplexing::REQUIRE_WAIT], \true);
        ...
        if ('2' === $protocolVersion || '2.0' === $protocolVersion) {
            if (!\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsHttp2()) {
                if ($requiredMultiplex) {
                    throw new \YoastSEO_Vendor\GuzzleHttp\Exception\ConnectException('Required multiplexing needs libcurl 8.14.0 or newer built with HTTP/2 support.', $request);
                }
    ...
    private static function normalizeMultiplex(array $options) : ?string
    {
        ...
        if (!\in_array($multiplex, [\YoastSEO_Vendor\GuzzleHttp\Multiplexing::NONE, \YoastSEO_Vendor\GuzzleHttp\Multiplexing::EAGER, \YoastSEO_Vendor\GuzzleHttp\Multiplexing::WAIT, \YoastSEO_Vendor\GuzzleHttp\Multiplexing::REQUIRE_EAGER, \YoastSEO_Vendor\GuzzleHttp\Multiplexing::REQUIRE_WAIT], \true)) {
    ...
    private static function assertRequiredMultiplexSupported(\YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy) : void
    {
        if (!\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsRequiredMultiplex()) {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\ConnectException('Required multiplexing needs libcurl 8.14.0 or newer built with HTTP/2 support.', $easy->request);
        }
    }
    /**
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function assertRequiredMultiplexRouteDirect(\YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy, array $conf) : void
    {
        ...
        if (self::getEffectiveProxy($conf) !== null || \defined('CURLOPT_PRE_PROXY') && ($conf[(int) \constant('CURLOPT_PRE_PROXY')] ?? '') !== '') {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\ConnectException('Required multiplexing cannot be guaranteed for cleartext requests sent through a proxy.', $easy->request);
        }
    }
    /**
     ...
     */
    private static function assertResolvedProxySupported(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, $proxyConf) : void
    {
        ...
        if ($scheme !== null && \preg_match('/^[a-z][a-z0-9.+-]*$/D', $scheme) !== 1) {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException('The proxy URL is malformed.', $request);
        }
        if ($scheme === 'https' && !\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsHttpsProxy()) {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException('HTTPS proxies are not supported by the installed libcurl; libcurl 7.52.0 or newer built with HTTPS-proxy support is required.', $request);
        }
    }
    /**
     * @return array{0: mixed, 1: string}
     */
    private static function resolveProxy(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array $options) : array
    {
        $proxyConf = null;
        $noProxyConf = '';
        if (isset($options['proxy'])) {
            if (!\is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    if (isset($options['proxy']['no']) && \YoastSEO_Vendor\GuzzleHttp\Utils::isUriInNoProxy($request->getUri(), $options['proxy']['no'])) {
                        $proxyConf = '';
                        $noProxyConf = '*';
                    } else {
        ...
        if ($proxyConf === null) {
            $proxyConf = \YoastSEO_Vendor\GuzzleHttp\Handler\ProxyEnvironment::getProxyForScheme($request->getUri()->getScheme());
            if ($proxyConf === null) {
                $proxyConf = '';
            } elseif (($noProxy = \YoastSEO_Vendor\GuzzleHttp\Handler\ProxyEnvironment::getNoProxy()) !== null && \YoastSEO_Vendor\GuzzleHttp\Utils::isUriInNoProxy($request->getUri(), \YoastSEO_Vendor\GuzzleHttp\Handler\ProxyEnvironment::splitNoProxy($noProxy))) {
                $proxyConf = '';
                $noProxyConf = '*';
            }
        }
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function rejectRequestLevelShareWithProxyAuth(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array $options, array $conf) : void
    {
        ...
        if (self::isSocksProxy($proxy, $conf)) {
            if (!\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsSocksProxyCredentialAwareConnectionReuse()) {
        ...
        if (\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsShareConnectionCaches()) {
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \YoastSEO_Vendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \YoastSEO_Vendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options) : void
    {
        ...
        if (\defined('CURLOPT_PROXYHEADER') && \array_key_exists((int) \constant('CURLOPT_PROXYHEADER'), $options['curl']) && !\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsProxyHeaderSeparation()) {
            \YoastSEO_Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.15', \sprintf('Passing %s in the "curl" request option on a build without proxy header separation support is deprecated; guzzlehttp/guzzle 8.0 will reject this configuration because proxy headers require libcurl 7.37.0 or newer built with proxy header separation support.', self::formatCurlOption((int) \constant('CURLOPT_PROXYHEADER'))));
        }
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            \YoastSEO_Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options) : void
    {
        if (\array_key_exists('stream_context', $options)) {
            \YoastSEO_Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
    ...
    public static function finish(callable $handler, \YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy, \YoastSEO_Vendor\GuzzleHttp\Handler\CurlFactoryInterface $factory) : \YoastSEO_Vendor\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        if (isset($easy->options['on_trailers'])) {
            try {
                ...
                return \YoastSEO_Vendor\GuzzleHttp\Promise\Create::rejectionFor(new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException('An error was encountered during the on_trailers event', $easy->request, $easy->response, $e));
            }
        }
    ...
    private static function finishError(callable $handler, \YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy, \YoastSEO_Vendor\GuzzleHttp\Handler\CurlFactoryInterface $factory) : \YoastSEO_Vendor\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $ctx[self::CURL_VERSION_STR] = \YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::getVersion() ?? '';
        $factory->release($easy);
    ...
    private static function createRejection(\YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy, array $ctx) : \YoastSEO_Vendor\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        if ($easy->createResponseException) {
            return \YoastSEO_Vendor\GuzzleHttp\Promise\Create::rejectionFor(new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException('An error was encountered while creating the response', $easy->request, null, $easy->createResponseException, $ctx));
        }
    ...
    private static function sanitizeCurlError(string $error, \YoastSEO_Vendor\Psr\Http\Message\UriInterface $uri, ?string $proxy = null) : string
    {
        if ('' === $error) {
            return $error;
        }
    ...
    private static function redactProxyUserInfo(string $error, ?string $proxy) : string
    {
        ...
        $redactedUserInfo = '***';
        try {
            $proxyUri = new \YoastSEO_Vendor\GuzzleHttp\Psr7\Uri($proxyForParsing);
            $redactedUserInfo = \YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($proxyUri)->getUserInfo();
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function forceFreshConnectionForAuthenticatedProxy(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array &$conf) : void
    {
        $proxy = self::getEffectiveProxy($conf);
     ...
     * @param array<int|string, mixed> $conf
     */
    private function isolateOpaqueShareAnonymousProxyTunnel(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array &$conf) : void
    {
        if (!$this->opaqueShareConnectionCache || !\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsShareConnectionCaches()) {
            return;
        }
        $proxy = self::getEffectiveProxy($conf);
    ...
    {
        if (\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsSocksProxyCredentialAwareConnectionReuse() || !\defined('CURLOPT_PRE_PROXY')) {
            return;
        }
    ...
    private static function proxyScheme(string $proxy) : ?string
    {
        ...
        return $position === \false ? null : \YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::asciiToLower(\substr($proxy, 0, $position));
    }
    /**
     * @param array<int|string, mixed> $conf
     */
    private static function requiresFreshConnectionForAuthenticatedProxy(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, string $proxy, array $conf) : bool
    {
        ...
        if (self::isSocksProxy($proxy, $conf)) {
            return !\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsSocksProxyCredentialAwareConnectionReuse();
        }
        if (!self::usesProxyTunnel($request, $conf) || !self::isHttpProxyForConnectionReuse($proxy, $conf)) {
            return \false;
        }
        ...
        if (!\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsProxyTlsCredentialAwareConnectionReuse() && self::hasCurlProxyTlsCredentials($conf)) {
            return \true;
        }
        if (\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsProxyCredentialAwareConnectionReuse()) {
            return \false;
        }
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function usesProxyTunnel(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array $conf) : bool
    {
        $scheme = $request->getUri()->getScheme();
        if ('https' === $scheme) {
            return \true;
    ...
    private static function isHttpProxyForConnectionReuse(string $proxy, array $conf) : bool
    {
        if (\strpos($proxy, '://') !== \false) {
            ...
            $proxyScheme = \YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::asciiToLower($proxyParts['scheme']);
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function applyProxyAuthorizationHeaderHandling(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array &$conf) : void
    {
        $proxy = self::getEffectiveProxy($conf);
        ...
        if (\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsProxyHeaderSeparation()) {
            if ($movedHeaders !== []) {
     ...
     */
    private static function applyManagedProxyAuthorization(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array &$conf, array $headers, bool $rawHttpHeadersReplaceManaged) : void
    {
        ...
        if (!\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsProxyHeaderSeparation()) {
            ...
            if ($proxy !== null && !self::isSocksProxy($proxy, $conf)) {
                throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException('Proxy-Authorization request headers through a possible HTTP or HTTPS proxy require libcurl 7.37.0 or newer built with proxy header separation support.', $request);
            }
            return;
        }
     ...
     * @return list<string>
     */
    private static function managedProxyAuthorizationHeaderLines(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request) : array
    {
        $headers = [];
    ...
    private static function isProxyAuthorizationHeaderLine(string $header) : bool
    {
        ...
        return \YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::caselessEquals(\trim(\substr($header, 0, $length), " \n\r\t\x00\v"), 'Proxy-Authorization');
    }
    private static function proxyAuthorizationHeaderValue(string $header) : ?string
    {
        ...
        if (!\YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::caselessEquals(\trim(\substr($header, 0, $position), " \n\r\t\x00\v"), 'Proxy-Authorization')) {
            return null;
        }
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function proxyTunnelSignature(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request, array $conf) : ?string
    {
        $proxy = self::getEffectiveProxy($conf);
        if ($proxy === null) {
            return null;
        ...
        if ($headerAuth === [] && \YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsProxyCredentialAwareConnectionReuse()) {
    ...
    {
        $uri = $easy->request->getUri();
        $protocols = \YoastSEO_Vendor\GuzzleHttp\Utils::normalizeProtocols($easy->options['protocols'] ?? ['http', 'https']);
        $scheme = $uri->getScheme();
        if (!\in_array($scheme, $protocols, \true)) {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException(\sprintf('The scheme "%s" is not allowed by the protocols request option.', $scheme), $easy->request);
        }
        if ($uri->getHost() === '') {
            throw new \YoastSEO_Vendor\GuzzleHttp\Exception\RequestException('URI must include a scheme and host. Use an absolute URI, a network-path reference starting with //, or configure a base_uri.', $easy->request);
        }
        ...
        if ('2' === $version || '2.0' === $version) {
            ...
            if (\in_array($multiplex, [\YoastSEO_Vendor\GuzzleHttp\Multiplexing::WAIT, \YoastSEO_Vendor\GuzzleHttp\Multiplexing::REQUIRE_WAIT], \true) && \YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsMultiplex()) {
            ...
            if (\in_array($multiplex, [\YoastSEO_Vendor\GuzzleHttp\Multiplexing::REQUIRE_EAGER, \YoastSEO_Vendor\GuzzleHttp\Multiplexing::REQUIRE_WAIT], \true)) {
                throw new \YoastSEO_Vendor\GuzzleHttp\Exception\ConnectException(\sprintf('The "multiplex" request option cannot be required for HTTP/%s requests; use protocol version 2.', $version), $easy->request);
            }
    ...
    private static function normalizeTlsFileType(string $option, $type) : string
    {
        ...
        return \YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper($type);
    }
    ...
    private function applyMethod(\YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy, array &$conf) : void
    {
        if ($easy->request->getMethod() === 'HEAD') {
            ...
            if (\YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::caselessEquals(\trim($easy->request->getHeaderLine('Expect'), " \n\r\t\x00\v"), '100-continue')) {
    ...
    private function removeHeader(string $name, array &$options) : void
    {
        foreach (\array_keys($options['_headers']) as $key) {
            if (\YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::caselessEquals((string) $key, $name)) {
    ...
    private function applyHandlerOptions(\YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy, array &$conf) : void
    {
        ...
        if ($timeoutRequiresNoSignal && \YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper(\substr(\PHP_OS, 0, 3)) !== 'WIN') {
        ...
        if (isset($options['cert'])) {
            ...
            if ($certType === null && \preg_match('#^(der|p12)$#iD', $ext)) {
                $conf[\CURLOPT_SSLCERTTYPE] = \YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper($ext);
            }
    ...
    private function applyTlsVersionRange(\YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy, array &$conf) : void
    {
        $options = $easy->options;
        ...
        if ($isHttp2 && $cryptoMethodMax !== null && \YoastSEO_Vendor\GuzzleHttp\Handler\TlsVersion::ordinal('crypto_method_max', $cryptoMethodMax) < 12) {
            throw new \InvalidArgumentException('Invalid crypto_method_max request option: HTTP/2 requires TLS 1.2 or higher');
        }
        if ($isHttp2 && $cryptoMethod !== null && \YoastSEO_Vendor\GuzzleHttp\Handler\TlsVersion::ordinal('crypto_method', $cryptoMethod) < 12) {
            $cryptoMethod = \STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT;
        }
        \YoastSEO_Vendor\GuzzleHttp\Handler\TlsVersion::assertRange($cryptoMethod, $cryptoMethodMax);
    ...
    private static function curlMinSslVersion($value) : int
    {
        ...
        if ($value === \STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT) {
            if (!\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsTls12()) {
                throw new \InvalidArgumentException('Invalid crypto_method request option: TLS 1.2 not supported by your version of cURL');
            }
            return \CURL_SSLVERSION_TLSv1_2;
        }
        if (\defined('STREAM_CRYPTO_METHOD_TLSv1_3_CLIENT') && $value === \STREAM_CRYPTO_METHOD_TLSv1_3_CLIENT) {
            if (!\YoastSEO_Vendor\GuzzleHttp\Handler\CurlVersion::supportsTls13()) {
    ...
    private static function validateRequestUriScheme(\YoastSEO_Vendor\Psr\Http\Message\RequestInterface $request) : void
    {
        $scheme = $request->getUri()->getScheme();
        if ($scheme === '') {
    ...
    private static function headersFromTrailerLines(array $lines) : array
    {
        ...
        foreach ($lines as $line) {
            ...
            $name = \YoastSEO_Vendor\GuzzleHttp\Psr7\Utils::asciiToLower(\trim($name, " \n\r\t\x00\v"));
    ...
    private function createHeaderFn(\YoastSEO_Vendor\GuzzleHttp\Handler\EasyHandle $easy) : callable
    {
        ...
        return static function ($ch, $h) use($onHeaders, $easy, &$startingResponse, &$collectingTrailers, $retainTrailers) {
            ...
            if ($h === "\r\n" || $h === "\n" || $h === "\r" || $h === '') {
                ...
                if ($easy->response !== null && !\YoastSEO_Vendor\GuzzleHttp\Handler\HeaderProcessor::isStatusLineCandidate($h)) {
                    ...
                    if ($retainTrailers && \YoastSEO_Vendor\GuzzleHttp\Handler\HeaderProcessor::isValidHeaderFieldLine($h)) {
                        $easy->trailers[] = $value;
                    }
                } else {
                    $collectingTrailers = \false;
