...
class PW_CMB2_Field_Select2 {
	...
	public function render_pw_select( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		$select_html = $field_type_object->select(array(
			'class'            => 'pwcmb2_select2 pwcmb2_select',
			'desc'             => $field_type_object->_desc(true),
			'options'          => '<option></option>' . $field_type_object->concat_items(),
			'data-placeholder' => esc_attr($field->args('attributes', 'placeholder') ? esc_attr($field->args('attributes', 'placeholder')) : esc_attr($field->args('description'))),
		));
	...
	public function render_pw_multiselect( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		echo wp_kses(sprintf( '<select%s>%s</select>%s', $attrs, $a['options'], $a['desc'] ), [
			'select' => [
				'multiple' => [],
				'style' => [],
				'class' => [],
	...
	public function setup_admin_scripts() {
		...
		wp_register_script( 'pw-select2', $asset_path . '/js/select2.min.js', array( 'jquery-ui-sortable' ), '4.0.13' );
		wp_enqueue_script( 'pw-select2-init', $asset_path . '/js/script.js', array( 'cmb2-scripts', 'pw-select2' ), self::VERSION );
		wp_register_style( 'pw-select2', $asset_path . '/css/select2.min.css', array(), '4.0.13' );
