...
class CMB2 extends CMB2_Base {
	...
	public function render_group_callback( $field_args, $field_group ) {
		...
		$con_id = isset($field_group->args('attributes')['data-conditional-id']) ? $field_group->args('attributes')['data-conditional-id'] : '';
		$con_value = isset($field_group->args('attributes')['data-conditional-value']) ? $field_group->args('attributes')['data-conditional-value'] : '';

		// Start building the HTML output
		$html = '<div class="cmb-row cmb-repeat-group-wrap ' . esc_attr($field_group->row_classes()) . '" data-fieldtype="group"><div class="cmb-td"><div data-groupid="' . esc_attr($field_group->id()) . '" id="' . esc_attr($field_group->id()) . '_repeat"';

		// Conditionally add 'data-conditional-id' and 'data-conditional-value' if they are set
		if (!empty($con_id)) {
			$html .= ' data-conditional-id="' . esc_attr($con_id) . '"';
		}
		if (!empty($con_value)) {
			$html .= ' data-conditional-value="' . esc_attr($con_value) . '"';
		}

		// Add any additional group attributes
		$html .= ' ' . $this->group_wrap_attributes($field_group);
