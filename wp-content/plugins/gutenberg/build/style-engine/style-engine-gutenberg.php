 ...
 * Example usage:
 *
 * $styles = gutenberg_style_engine_get_styles( array( 'color' => array( 'text' => '#cccccc' ) ) );
 ...
 * }
 */
function gutenberg_style_engine_get_styles( $block_styles, $options = array() ) {
	...
	$parsed_styles = WP_Style_Engine_Gutenberg::parse_block_styles( $block_styles, $options );

	// Output.
	$styles_output = array();

	if ( ! empty( $parsed_styles['declarations'] ) ) {
		$styles_output['css']          = WP_Style_Engine_Gutenberg::compile_css( $parsed_styles['declarations'], $options['selector'] );
		$styles_output['declarations'] = $parsed_styles['declarations'];
		if ( ! empty( $options['context'] ) ) {
			WP_Style_Engine_Gutenberg::store_css_rule( $options['context'], $options['selector'], $parsed_styles['declarations'] );
		}
	}
 ...
 * $css       = gutenberg_style_engine_get_stylesheet_from_css_rules( $css_rules );
 ...
 */
function gutenberg_style_engine_get_stylesheet_from_css_rules( $css_rules, $options = array() ) {
	if ( empty( $css_rules ) ) {
		return '';
	}
	...
	foreach ( $css_rules as $css_rule ) {
		...
		if ( ! empty( $options['context'] ) ) {
			WP_Style_Engine_Gutenberg::store_css_rule( $options['context'], $css_rule['selector'], $css_rule['declarations'], $at_rule );
		}

		$css_rule_objects[] = new WP_Style_Engine_CSS_Rule_Gutenberg( $css_rule['selector'], $css_rule['declarations'], $at_rule );
	}
	...
	return WP_Style_Engine_Gutenberg::compile_stylesheet_from_css_rules( $css_rule_objects, $options );
}
 ...
 */
function gutenberg_style_engine_get_stylesheet_from_context( $context, $options = array() ) {
	if ( empty( $context ) ) {
		return '';
	}

	return WP_Style_Engine_Gutenberg::compile_stylesheet_from_css_rules( WP_Style_Engine_Gutenberg::get_store( $context )->get_all_rules(), $options );
}
