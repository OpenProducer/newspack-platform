...
function gutenberg_render_block_core_image( $attributes, $content, $block ) {
	...
	if ( $lightbox_enabled ) {
		return gutenberg_block_core_image_render_lightbox( $processor->get_updated_html(), $block->parsed_block );
	}
 ...
 */
function gutenberg_block_core_image_ensure_interactivity_dependency() {
	global $wp_scripts;
	if (
...
add_action( 'wp_print_scripts', 'gutenberg_block_core_image_ensure_interactivity_dependency' );
