...
global $gutenberg_connectors_routes, $gutenberg_connectors_menu_items;
$gutenberg_connectors_routes     = array();
$gutenberg_connectors_menu_items = array();

if ( ! function_exists( 'gutenberg_register_connectors_route' ) ) {
	/**
	 * Register a route for the connectors page.
	 *
	 ...
	 */
	function gutenberg_register_connectors_route( $path, $content_module = null, $route_module = null ) {
		global $gutenberg_connectors_routes;
		...
		$gutenberg_connectors_routes[] = $route;
	}
}

if ( ! function_exists( 'gutenberg_register_connectors_menu_item' ) ) {
	/**
	 * Register a menu item for the connectors page.
	 *
	 ...
	 */
	function gutenberg_register_connectors_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
		global $gutenberg_connectors_menu_items;
		...
		$gutenberg_connectors_menu_items[] = $menu_item;
	}
}

if ( ! function_exists( 'gutenberg_get_connectors_routes' ) ) {
	/**
	 * Get all registered routes for the connectors page.
	 *
	 * @return array Array of route objects.
	 */
	function gutenberg_get_connectors_routes() {
		global $gutenberg_connectors_routes;
		return $gutenberg_connectors_routes ?? array();
	}
}

if ( ! function_exists( 'gutenberg_get_connectors_menu_items' ) ) {
	/**
	 * Get all registered menu items for the connectors page.
	 *
	 * @return array Array of menu item objects.
	 */
	function gutenberg_get_connectors_menu_items() {
		global $gutenberg_connectors_menu_items;
		return $gutenberg_connectors_menu_items ?? array();
	}
}

if ( ! function_exists( 'gutenberg_connectors_preload_data' ) ) {
	/**
	 * Preload REST API data for the connectors page.
	 * Automatically called during page rendering.
	 */
	function gutenberg_connectors_preload_data() {
...
if ( ! function_exists( 'gutenberg_connectors_render_page' ) ) {
	/**
	 * Render the connectors page.
	 * Call this function from add_menu_page or add_submenu_page.
	 */
	function gutenberg_connectors_render_page() {
		...
		gutenberg_connectors_preload_data();

		// Get all registered routes and menu items
		$menu_items = gutenberg_get_connectors_menu_items();
		$routes = gutenberg_get_connectors_routes();
		...
		if ( file_exists( $asset_file ) ) {
			...
			wp_register_script( 'connectors-prerequisites', '', $asset['dependencies'], $asset['version'], true );
			...
			wp_register_style( 'connectors-prerequisites', false, $style_dependencies, $asset['version'] );

			// Build dependencies for connectors module
			...
			wp_enqueue_script( 'connectors-prerequisites' );
			wp_enqueue_script_module( 'connectors' );
			wp_enqueue_style( 'connectors-prerequisites' );
		}
		...
		<body class="connectors">
			<div id="connectors-app" style="height: 100vh; box-sizing: border-box;"></div>
		<?php
...
add_action( 'admin_init', 'gutenberg_connectors_intercept_render' );
