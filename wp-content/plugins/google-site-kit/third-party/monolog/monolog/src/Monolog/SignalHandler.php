...
class SignalHandler
{
     ...
     */
    public function registerSignalHandler(int $signo, $level = \Google\Site_Kit_Dependencies\Psr\Log\LogLevel::CRITICAL, bool $callPrevious = \true, bool $restartSyscalls = \true, ?bool $async = \true) : self
    {
        if (!\extension_loaded('pcntl') || !\function_exists('pcntl_signal')) {
            return $this;
        }
        $level = \Google\Site_Kit_Dependencies\Monolog\Logger::toMonologLevel($level);
        if ($callPrevious) {
    ...
    public function handleSignal(int $signo, $siginfo = null) : void
    {
        ...
        $level = $this->signalLevelMap[$signo] ?? \Google\Site_Kit_Dependencies\Psr\Log\LogLevel::CRITICAL;
