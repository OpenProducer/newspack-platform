...
class BrowserConsoleHandler extends \Google\Site_Kit_Dependencies\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return [\Google\Site_Kit_Dependencies\Monolog\Logger::DEBUG => 'debug', \Google\Site_Kit_Dependencies\Monolog\Logger::INFO => 'info', \Google\Site_Kit_Dependencies\Monolog\Logger::NOTICE => 'info', \Google\Site_Kit_Dependencies\Monolog\Logger::WARNING => 'warn', \Google\Site_Kit_Dependencies\Monolog\Logger::ERROR => 'error', \Google\Site_Kit_Dependencies\Monolog\Logger::CRITICAL => 'error', \Google\Site_Kit_Dependencies\Monolog\Logger::ALERT => 'error', \Google\Site_Kit_Dependencies\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \Google\Site_Kit_Dependencies\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \Google\Site_Kit_Dependencies\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \Google\Site_Kit_Dependencies\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
