 ...
 */
namespace Google\Site_Kit_Dependencies\phpseclib3\Net;

use Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\Blowfish;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\ChaCha20;
...
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\Common\SymmetricKey;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\DH;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\DSA;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\EC;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\Random;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\RC4;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\Rijndael;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\RSA;
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\TripleDES;
// Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use Google\Site_Kit_Dependencies\phpseclib3\Crypt\Twofish;
use Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException;
use Google\Site_Kit_Dependencies\phpseclib3\Exception\InsufficientSetupException;
use Google\Site_Kit_Dependencies\phpseclib3\Exception\InvalidPacketLengthException;
use Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException;
use Google\Site_Kit_Dependencies\phpseclib3\Exception\TimeoutException;
use Google\Site_Kit_Dependencies\phpseclib3\Exception\UnableToConnectException;
...
use Google\Site_Kit_Dependencies\phpseclib3\System\SSH\Agent;
/**
 * Pure-PHP implementation of SSHv2.
 *
 * @author  Jim Wigginton <terrafrost@php.net>
...
class SSH2
{
    ...
    public function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        if (\Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::is_stringable($host)) {
            $this->host = $host;
            $this->port = $port;
        }
    }
    ...
    private function connect()
    {
        ...
        if (!\is_resource($this->fsock)) {
            ...
            if (!$this->fsock) {
                ...
                throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\UnableToConnectException(\rtrim("Cannot connect to {$host}. Error {$errno}. {$errstr}"));
            }
            $elapsed = \microtime(\true) - $start;
            if ($this->curTimeout) {
            ...
            if (\defined('Google\\Site_Kit_Dependencies\\NET_SSH2_LOGGING')) {
        ...
        if (\feof($this->fsock)) {
            $this->bitmap = 0;
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Connection closed by server; are you sure you\'re connected to an SSH server?');
        }
        $extra = $matches[1];
        ...
        if (\version_compare($matches[3], '1.99', '<')) {
            $this->bitmap = 0;
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\UnableToConnectException("Cannot connect to SSH {$matches[3]} servers");
        }
    ...
    private function key_exchange($kexinit_payload_server = \false)
    {
        ...
        $kex_algorithms = isset($preferred['kex']) ? $preferred['kex'] : \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2::getSupportedKEXAlgorithms();
        $server_host_key_algorithms = isset($preferred['hostkey']) ? $preferred['hostkey'] : \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2::getSupportedHostKeyAlgorithms();
        $s2c_encryption_algorithms = isset($preferred['server_to_client']['crypt']) ? $preferred['server_to_client']['crypt'] : \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2::getSupportedEncryptionAlgorithms();
        $c2s_encryption_algorithms = isset($preferred['client_to_server']['crypt']) ? $preferred['client_to_server']['crypt'] : \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2::getSupportedEncryptionAlgorithms();
        $s2c_mac_algorithms = isset($preferred['server_to_client']['mac']) ? $preferred['server_to_client']['mac'] : \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2::getSupportedMACAlgorithms();
        $c2s_mac_algorithms = isset($preferred['client_to_server']['mac']) ? $preferred['client_to_server']['mac'] : \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2::getSupportedMACAlgorithms();
        $s2c_compression_algorithms = isset($preferred['server_to_client']['comp']) ? $preferred['server_to_client']['comp'] : \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2::getSupportedCompressionAlgorithms();
        $c2s_compression_algorithms = isset($preferred['client_to_server']['comp']) ? $preferred['client_to_server']['comp'] : \Google\Site_Kit_Dependencies\phpseclib3\Net\SSH2::getSupportedCompressionAlgorithms();
        ...
        $client_cookie = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Random::string(16);
        $kexinit_payload_client = \pack('Ca*', NET_SSH2_MSG_KEXINIT, $client_cookie);
        $kexinit_payload_client .= \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2(
            'L10bN',
            $kex_algorithms,
        ...
        if (!$decrypt || ($decryptKeyLength = $this->encryption_algorithm_to_key_size($decrypt)) === null) {
            ...
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible server to client encryption algorithms found');
        }
        ...
        if (!$encrypt || ($encryptKeyLength = $this->encryption_algorithm_to_key_size($encrypt)) === null) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible client to server encryption algorithms found');
        }
        // through diffie-hellman key exchange a symmetric key is obtained
        $this->kex_algorithm = self::array_intersect_first($kex_algorithms, $this->kex_algorithms);
        if ($this->kex_algorithm === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible key exchange algorithms found');
        }
        $server_host_key_algorithm = self::array_intersect_first($server_host_key_algorithms, $this->server_host_key_algorithms);
        if ($server_host_key_algorithm === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible server host key algorithms found');
        }
        $mac_algorithm_out = self::array_intersect_first($c2s_mac_algorithms, $this->mac_algorithms_client_to_server);
        if ($mac_algorithm_out === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible client to server message authentication algorithms found');
        }
        $mac_algorithm_in = self::array_intersect_first($s2c_mac_algorithms, $this->mac_algorithms_server_to_client);
        if ($mac_algorithm_in === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible server to client message authentication algorithms found');
        }
        ...
        if ($compression_algorithm_in === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible server to client compression algorithms found');
        }
        ...
        if ($compression_algorithm_out === \false) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('No compatible client to server compression algorithms found');
        }
        ...
        switch ($this->kex_algorithm) {
            ...
            case 'ecdh-sha2-nistp521':
                $kexHash = new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('sha512');
                break;
            case 'ecdh-sha2-nistp384':
                $kexHash = new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('sha384');
                break;
            ...
            case 'curve25519-sha256':
                $kexHash = new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('sha256');
                break;
            default:
                $kexHash = new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('sha1');
        }
        ...
        if (\strpos($this->kex_algorithm, 'curve25519-sha256') === 0 || \strpos($this->kex_algorithm, 'ecdh-sha2-nistp') === 0) {
            ...
            $ourPrivate = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\EC::createKey($curve);
            ...
            if (\strpos($this->kex_algorithm, 'diffie-hellman-group-exchange') === 0) {
                ...
                list($type, $primeBytes, $gBytes) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Css', $response);
                $this->updateLogHistory('NET_SSH2_MSG_KEXDH_REPLY', 'NET_SSH2_MSG_KEXDH_GEX_GROUP');
                $prime = new \Google\Site_Kit_Dependencies\phpseclib3\Math\BigInteger($primeBytes, -256);
                $g = new \Google\Site_Kit_Dependencies\phpseclib3\Math\BigInteger($gBytes, -256);
                $exchange_hash_rfc4419 = $dh_group_sizes_packed . \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $primeBytes, $gBytes);
                $params = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\DH::createParameters($prime, $g);
                $clientKexInitMessage = 'NET_SSH2_MSG_KEXDH_GEX_INIT';
                $serverKexReplyMessage = 'NET_SSH2_MSG_KEXDH_GEX_REPLY';
            } else {
                $params = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\DH::createParameters($this->kex_algorithm);
                $clientKexInitMessage = 'NET_SSH2_MSG_KEXDH_INIT';
                $serverKexReplyMessage = 'NET_SSH2_MSG_KEXDH_REPLY';
            }
            $keyLength = \min($kexHash->getLengthInBytes(), \max($encryptKeyLength, $decryptKeyLength));
            $ourPrivate = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\DH::createKey($params, 16 * $keyLength);
            // 2 * 8 * $keyLength
        ...
        list($type, $server_public_host_key, $theirPublicBytes, $this->signature) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Csss', $response);
        ...
        list($public_key_format) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $server_public_host_key);
        ...
        $keyBytes = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\DH::computeSecret($ourPrivate, $theirPublicBytes);
        if (($keyBytes & "\xff\x80") === "\x00\x00") {
            $keyBytes = \substr($keyBytes, 1);
        } elseif (($keyBytes[0] & "\x80") === "\x80") {
            $keyBytes = "\x00{$keyBytes}";
        }
        $this->exchange_hash = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s5', $this->identifier, $this->server_identifier, $kexinit_payload_client, $kexinit_payload_server, $this->server_public_host_key);
        $this->exchange_hash .= $exchange_hash_rfc4419;
        $this->exchange_hash .= \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s3', $ourPublicBytes, $theirPublicBytes, $keyBytes);
        ...
        if ($this->hmac_create instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash) {
        ...
        if ($this->hmac_check instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash) {
    ...
    private static function encryption_algorithm_to_crypt_instance($algorithm)
    {
        switch ($algorithm) {
            case '3des-cbc':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\TripleDES('cbc');
            case '3des-ctr':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\TripleDES('ctr');
            case 'aes256-cbc':
            case 'aes192-cbc':
            case 'aes128-cbc':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Rijndael('cbc');
            case 'aes256-ctr':
            case 'aes192-ctr':
            case 'aes128-ctr':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Rijndael('ctr');
            case 'blowfish-cbc':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Blowfish('cbc');
            case 'blowfish-ctr':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Blowfish('ctr');
            case 'twofish128-cbc':
            case 'twofish192-cbc':
            case 'twofish256-cbc':
            case 'twofish-cbc':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Twofish('cbc');
            case 'twofish128-ctr':
            case 'twofish192-ctr':
            case 'twofish256-ctr':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Twofish('ctr');
            case 'arcfour':
            case 'arcfour128':
            case 'arcfour256':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\RC4();
            case 'aes128-gcm@openssh.com':
            case 'aes256-gcm@openssh.com':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Rijndael('gcm');
            case 'chacha20-poly1305@openssh.com':
                return new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\ChaCha20();
        }
        return null;
    }
    /**
    ...
    private static function mac_algorithm_to_hash_instance($algorithm)
    {
        switch ($algorithm) {
            ...
            case 'umac-64-etm@openssh.com':
                return [new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('umac-64'), 16];
            case 'umac-128@openssh.com':
            case 'umac-128-etm@openssh.com':
                return [new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('umac-128'), 16];
            case 'hmac-sha2-512':
            case 'hmac-sha2-512-etm@openssh.com':
                return [new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('sha512'), 64];
            case 'hmac-sha2-256':
            case 'hmac-sha2-256-etm@openssh.com':
                return [new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('sha256'), 32];
            case 'hmac-sha1':
            case 'hmac-sha1-etm@openssh.com':
                return [new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('sha1'), 20];
            case 'hmac-sha1-96':
                return [new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('sha1-96'), 20];
            case 'hmac-md5':
                return [new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('md5'), 16];
            case 'hmac-md5-96':
                return [new \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash('md5-96'), 16];
        }
    }
    /**
    ...
    protected function sublogin($username, ...$args)
    {
        ...
        foreach ($args as $arg) {
            switch (\true) {
                case $arg instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Common\PublicKey:
                    throw new \UnexpectedValueException('A PublicKey object was passed to the login method instead of a PrivateKey object');
                case $arg instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Common\PrivateKey:
                case $arg instanceof \Google\Site_Kit_Dependencies\phpseclib3\System\SSH\Agent:
                case \is_array($arg):
                case \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::is_stringable($arg):
                    break;
                default:
                    ...
                    switch ($method) {
                        case 'publickey':
                            foreach ($args as $key => $arg) {
                                if ($arg instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Common\PrivateKey || $arg instanceof \Google\Site_Kit_Dependencies\phpseclib3\System\SSH\Agent) {
                                    $newargs[] = $arg;
                                    unset($args[$key]);
                                    break;
                                }
                        ...
                        case 'keyboard-interactive':
                            ...
                            foreach ($args as $arg) {
                                ...
                                if ($hasString || \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::is_stringable($arg)) {
                                    $hasString = \true;
                                    break;
                                }
                            }
    ...
    private function login_helper($username, $password = null)
    {
        ...
        if (!($this->bitmap & self::MASK_LOGIN_REQ)) {
            $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Cs', NET_SSH2_MSG_SERVICE_REQUEST, 'ssh-userauth');
            $this->send_binary_packet($packet);
            try {
                $response = $this->get_binary_packet_or_close(NET_SSH2_MSG_SERVICE_ACCEPT);
            } catch (\Google\Site_Kit_Dependencies\phpseclib3\Exception\InvalidPacketLengthException $e) {
            ...
            list($type) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('C', $response);
            list($service) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
        ...
        if (\strlen($this->last_interactive_response)) {
            return !\Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::is_stringable($password) && !\is_array($password) ? \false : $this->keyboard_interactive_process($password);
        }
        if ($password instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Common\PrivateKey) {
            return $this->privatekey_login($username, $password);
        }
        if ($password instanceof \Google\Site_Kit_Dependencies\phpseclib3\System\SSH\Agent) {
        ...
        if (!isset($password)) {
            $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Cs3', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'none');
            $this->send_binary_packet($packet);
            ...
            switch ($type) {
                ...
                case NET_SSH2_MSG_USERAUTH_FAILURE:
                    list($auth_methods) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('L', $response);
        ...
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Cs3bs', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'password', \false, $password);
        // remove the username and password from the logged packet
        if (!\defined('Google\\Site_Kit_Dependencies\\NET_SSH2_LOGGING')) {
            $logged = null;
        } else {
            $logged = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Cs3bs', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'password', \false, 'password');
        }
        ...
        switch ($type) {
            ...
            case NET_SSH2_MSG_USERAUTH_FAILURE:
                ...
                list($auth_methods, $partial_success) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Lb', $response);
                $this->auth_methods_to_continue = $auth_methods;
    ...
    {
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2(
            'Cs5',
    ...
    private function keyboard_interactive_process(...$responses)
    {
        ...
        switch ($type) {
            case NET_SSH2_MSG_USERAUTH_INFO_REQUEST:
                list(, , , $num_prompts) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s3N', $response);
                ...
                if (isset($this->keyboard_requests_responses)) {
                    for ($i = 0; $i < $num_prompts; $i++) {
                        list($prompt, ) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('sC', $response);
                ...
                */
                ...
                for ($i = 0; $i < \count($responses); $i++) {
                    $packet .= \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $responses[$i]);
                    $logged .= \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', 'dummy-answer');
                }
                $this->send_binary_packet($packet, $logged);
     ...
     * @return bool
     */
    private function ssh_agent_login($username, \Google\Site_Kit_Dependencies\phpseclib3\System\SSH\Agent $agent)
    {
        $this->agent = $agent;
     ...
     * @throws \RuntimeException on connection error
     */
    private function privatekey_login($username, \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Common\PrivateKey $privatekey)
    {
        $publickey = $privatekey->getPublicKey();
        if ($publickey instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\RSA) {
            $privatekey = $privatekey->withPadding(\Google\Site_Kit_Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
        ...
        } elseif ($publickey instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\EC) {
            ...
            switch ($curveName) {
                ...
                default:
                    if (\is_array($curveName)) {
                        throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\UnsupportedCurveException('Specified Curves are not supported by SSH2');
                    }
                    throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\UnsupportedCurveException('Named Curve of ' . $curveName . ' is not supported by phpseclib3\'s SSH2 implementation');
            }
        } elseif ($publickey instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\DSA) {
            $privatekey = $privatekey->withSignatureFormat('SSH2');
            $hash = 'sha1';
            $signatureType = 'ssh-dss';
        } else {
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException('Please use either an RSA key, an EC one or a DSA key');
        }
        $publickeyStr = $publickey->toString('OpenSSH', ['binary' => \true]);
        $part1 = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('Csss', NET_SSH2_MSG_USERAUTH_REQUEST, $username, 'ssh-connection', 'publickey');
        $part2 = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $signatureType, $publickeyStr);
        ...
        $signature = $privatekey->sign(\Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $this->session_id) . $packet);
        if ($publickey instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\RSA) {
            $signature = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('ss', $signatureType, $signature);
        }
        $packet .= \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('s', $signature);
        $this->send_binary_packet($packet);
    ...
    public function exec($command, $callback = null)
    {
        ...
        if ($this->request_pty === \true) {
            ...
            $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsCsN4s', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_EXEC], 'pty-req', 1, $this->term, $this->windowColumns, $this->windowRows, 0, 0, $terminal_modes);
            $this->send_binary_packet($packet);
        ...
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsCs', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_EXEC], 'exec', 1, $command);
        $this->send_binary_packet($packet);
        $this->channel_status[self::CHANNEL_EXEC] = NET_SSH2_MSG_CHANNEL_REQUEST;
        if (!$this->get_channel_packet(self::CHANNEL_EXEC)) {
            return \false;
    ...
    protected function open_channel($channel, $skip_extended = \false)
    {
        ...
        $packet_size = 0x4000;
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CsN3', NET_SSH2_MSG_CHANNEL_OPEN, 'session', $channel, $this->window_size_server_to_client[$channel], $packet_size);
        $this->send_binary_packet($packet);
    ...
    {
        if (!$this->isAuthenticated()) {
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\InsufficientSetupException('Operation disallowed prior to login()');
        }
        ...
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsb', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_SHELL], 'shell', \true);
        $this->send_binary_packet($packet);
    ...
    public function requestAgentForwarding()
    {
        ...
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsC', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[$request_channel], 'auth-agent-req@openssh.com', 1);
    ...
    public function read($expect = '', $mode = self::READ_SIMPLE, $channel = null)
    {
        ...
        if (!$this->is_channel_status_data($channel) && empty($this->channel_buffers[$channel])) {
            if ($channel != self::CHANNEL_SHELL) {
                throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\InsufficientSetupException('Data is not available on channel');
        ...
        while (\true) {
            ...
            if ($pos !== \false) {
                return \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($this->interactiveBuffer, $pos + \strlen($match));
            }
            $response = $this->get_channel_packet($channel);
            if ($response === \true) {
                return \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($this->interactiveBuffer, \strlen($this->interactiveBuffer));
            }
    ...
    public function startSubsystem($subsystem)
    {
        ...
        $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNsCs', NET_SSH2_MSG_CHANNEL_REQUEST, $this->server_channels[self::CHANNEL_SUBSYSTEM], 'subsystem', 1, $subsystem);
        $this->send_binary_packet($packet);
    ...
    private function get_binary_packet_or_close(...$message_types)
    {
        try {
            ...
            if (\count($message_types) > 0 && !\in_array(\ord($packet[0]), $message_types)) {
                $this->disconnect_helper(NET_SSH2_DISCONNECT_PROTOCOL_ERROR);
                throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Bad message type. Expected: #' . \implode(', #', $message_types) . '. Got: #' . \ord($packet[0]));
            }
            return $packet;
        } catch (\Google\Site_Kit_Dependencies\phpseclib3\Exception\TimeoutException $e) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_BY_APPLICATION);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Connection closed due to timeout');
        }
    }
    /**
     * Gets Binary Packets
    ...
    private function get_binary_packet()
    {
        ...
        while (\strlen($packet->raw) < $packet->size) {
            if (\feof($this->fsock)) {
                ...
                throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Connection closed by server');
            }
            if ($this->curTimeout < 0) {
                $this->is_timeout = \true;
                throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\TimeoutException('Timed out waiting for server');
            }
        ...
        $raw = $packet->raw;
        if ($this->hmac_check instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash) {
            $hmac = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::pop($raw, $this->hmac_size);
        }
        ...
        if ($this->decrypt) {
            switch ($this->decryptName) {
                ...
                case 'aes256-gcm@openssh.com':
                    ...
                    \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::increment_str($this->decryptInvocationCounter);
                    $this->decrypt->setAAD(\Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($raw, $packet_length_header_size));
                    $this->decrypt->setTag(\Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::pop($raw, $this->decrypt_block_size));
                    $packet->plain = $this->decrypt->decrypt($raw);
                    break;
                case 'chacha20-poly1305@openssh.com':
                    // This should be impossible, but we are checking anyway to narrow the type for Psalm.
                    if (!$this->decrypt instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\ChaCha20) {
                        throw new \LogicException('$this->decrypt is not a ' . \Google\Site_Kit_Dependencies\phpseclib3\Crypt\ChaCha20::class);
                    }
                    ...
                    $this->decrypt->setTag(\Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::pop($raw, 16));
                    $packet->plain = $this->decrypt->decrypt($raw);
                    break;
                default:
                    if (!$this->hmac_check instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash || !$this->hmac_check_etm) {
                        // first block was already decrypted for contained packet_length header
                        \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($raw, $this->decrypt_block_size);
                        if (\strlen($raw) > 0) {
                            $packet->plain .= $this->decrypt->decrypt($raw);
                        }
                    } else {
                        \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($raw, $packet_length_header_size);
                        $packet->plain = $this->decrypt->decrypt($raw);
                    }
                    break;
            }
        ...
        if ($this->hmac_check instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash) {
            ...
            if (($this->hmac_check->getHash() & "\xff\xff\xff\xff") == 'umac') {
                ...
                if ($hmac != $this->hmac_check->hash($reconstructed)) {
                    ...
                    throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Invalid UMAC');
                }
            } else {
                if ($hmac != $this->hmac_check->hash(\pack('Na*', $this->get_seq_no, $reconstructed))) {
                    $this->disconnect_helper(NET_SSH2_DISCONNECT_MAC_ERROR);
                    throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Invalid HMAC');
                }
            }
        }
        $padding_length = 0;
        $payload = $packet->plain;
        \extract(\unpack('Cpadding_length', \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($payload, 1)));
        if ($padding_length > 0) {
            \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::pop($payload, $padding_length);
        }
        ...
        if (empty($payload)) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_PROTOCOL_ERROR);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Plaintext is too short');
        }
        switch ($this->decompress) {
            ...
            case self::NET_SSH2_COMPRESSION_ZLIB:
                if ($this->regenerate_decompression_context) {
                    ...
                    if ($cm != 8) {
                        // deflate
                        throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException("Only CM = 8 ('deflate') is supported ({$cm})");
                    }
                    $cinfo = ($cmf & 0xf0) >> 4;
                    if ($cinfo > 7) {
    ...
    private function get_binary_packet_size(&$packet)
    {
            ...
            switch ($this->decryptName) {
                ...
                default:
                    if (!$this->hmac_check instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash || !$this->hmac_check_etm) {
                        ...
                        \extract(\unpack('Npacket_length', \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($packet->plain, $packet_length_header_size)));
        ...
        if ($packet_length <= 0 || $packet_length > 0x9000 || ($packet_length + $added_validation_length) % $this->decrypt_block_size != 0) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_PROTOCOL_ERROR);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\InvalidPacketLengthException('Invalid packet length');
        }
        if ($this->hmac_check instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash) {
    ...
    {
        \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($payload, 1);
        list($reason_code, $message) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Ns', $payload);
    ...
    private function filter($payload)
    {
        switch (\ord($payload[0])) {
            ...
            case NET_SSH2_MSG_DEBUG:
                \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($payload, 2);
                // second byte is "always_display"
                list($message) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $payload);
            ...
            case NET_SSH2_MSG_KEXINIT:
                ...
                if ($this->session_id !== \false) {
                    if (!$this->key_exchange($payload)) {
                        $this->disconnect_helper(NET_SSH2_DISCONNECT_KEY_EXCHANGE_FAILED);
                        throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Key exchange failed');
                    }
                    $payload = $this->get_binary_packet();
                }
                break;
            case NET_SSH2_MSG_EXT_INFO:
                \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($payload, 1);
                list($nr_extensions) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $payload);
                for ($i = 0; $i < $nr_extensions; $i++) {
                    list($extension_name, $extension_value) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $payload);
        ...
        if ($this->bitmap & self::MASK_CONNECTED && !$this->isAuthenticated() && \ord($payload[0]) == NET_SSH2_MSG_USERAUTH_BANNER) {
            \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($payload, 1);
            list($this->banner_message) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $payload);
            $payload = $this->get_binary_packet();
        }
        ...
        if ($this->bitmap & self::MASK_CONNECTED && $this->isAuthenticated()) {
            switch (\ord($payload[0])) {
                ...
                case NET_SSH2_MSG_GLOBAL_REQUEST:
                    ...
                    \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($payload, 1);
                    list($request_name) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $payload);
                ...
                case NET_SSH2_MSG_CHANNEL_OPEN:
                    ...
                    \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($payload, 1);
                    list($data, $server_channel) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('sN', $payload);
                    switch ($data) {
                        case 'auth-agent':
                        case 'auth-agent@openssh.com':
                            if (isset($this->agent)) {
                                $new_channel = self::CHANNEL_AGENT_FORWARD;
                                list($remote_window_size, $remote_maximum_packet_size) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $payload);
    ...
    protected function get_channel_packet($client_channel, $skip_extended = \false)
    {
        ...
        while (\true) {
            ...
            if (\strlen($response) >= 4) {
                list($channel) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
            }
            ...
            if (isset($channel) && isset($this->channel_status[$channel]) && isset($this->window_size_server_to_client[$channel])) {
                ...
                switch ($type) {
                    case NET_SSH2_MSG_CHANNEL_WINDOW_ADJUST:
                        list($window_size) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('N', $response);
                    ...
                    case NET_SSH2_MSG_CHANNEL_EXTENDED_DATA:
                        ...
                        */
                        ...
                        list($data_type_code, $data) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('Ns', $response);
                    ...
                    case NET_SSH2_MSG_CHANNEL_REQUEST:
                        ...
                        list($value) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                        switch ($value) {
                            case 'exit-signal':
                                list(, $signal_name, , $error_message) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('bsbs', $response);
                                ...
                                continue 3;
                            case 'exit-status':
                                list(, $this->exit_status) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('CN', $response);
                    ...
                    case NET_SSH2_MSG_CHANNEL_OPEN:
                        switch ($type) {
                            case NET_SSH2_MSG_CHANNEL_OPEN_CONFIRMATION:
                                list($this->server_channels[$channel], $window_size, $this->packet_size_client_to_server[$channel]) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NNN', $response);
                                if ($window_size < 0) {
                    ...
                    case NET_SSH2_MSG_CHANNEL_REQUEST:
                        switch ($type) {
                            ...
                            case NET_SSH2_MSG_CHANNEL_DATA:
                                list($data) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $response);
                                $this->channel_buffers[$channel][] = \chr($type) . $data;
                                return $this->get_channel_packet($client_channel, $skip_extended);
                            default:
                                $this->disconnect_helper(NET_SSH2_DISCONNECT_BY_APPLICATION);
    ...
    protected function send_binary_packet($data, $logged = null)
    {
        if (!\is_resource($this->fsock) || \feof($this->fsock)) {
            $this->disconnect_helper(NET_SSH2_DISCONNECT_CONNECTION_LOST);
            throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\ConnectionClosedException('Connection closed prematurely');
        }
        if (!isset($logged)) {
            $logged = $data;
        }
        ...
        switch (\true) {
            ...
            case $this->hmac_create instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash && $this->hmac_create_etm:
                $padding_length += 4;
                $packet_length += 4;
        }
        $padding = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Random::string($padding_length);
        // we subtract 4 from packet_length because the packet_length field isn't supposed to include itself
        $packet = \pack('NCa*', $packet_length - 4, $padding_length, $data . $padding);
        $hmac = '';
        if ($this->hmac_create instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash && !$this->hmac_create_etm) {
        ...
        if ($this->encrypt) {
            switch ($this->encryptName) {
                ...
                case 'aes256-gcm@openssh.com':
                    ...
                    \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::increment_str($this->encryptInvocationCounter);
                    $this->encrypt->setAAD($temp = $packet & "\xff\xff\xff\xff");
                    $packet = $temp . $this->encrypt->encrypt(\substr($packet, 4));
                    break;
                case 'chacha20-poly1305@openssh.com':
                    // This should be impossible, but we are checking anyway to narrow the type for Psalm.
                    if (!$this->encrypt instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\ChaCha20) {
                        throw new \LogicException('$this->encrypt is not a ' . \Google\Site_Kit_Dependencies\phpseclib3\Crypt\ChaCha20::class);
                    }
                    ...
                    break;
                default:
                    $packet = $this->hmac_create instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash && $this->hmac_create_etm ? ($packet & "\xff\xff\xff\xff") . $this->encrypt->encrypt(\substr($packet, 4)) : $this->encrypt->encrypt($packet);
            }
        }
        if ($this->hmac_create instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Hash && $this->hmac_create_etm) {
            if (($this->hmac_create->getHash() & "\xff\xff\xff\xff") == 'umac') {
    ...
    protected function append_log_helper($constant, $message_number, $message, array &$message_number_log, array &$message_log, &$log_size, &$realtime_log_file, &$realtime_log_wrap, &$realtime_log_size)
    {
        ...
        if (\strlen($message_number) > 2) {
            \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($message);
        }
        switch ($constant) {
    ...
    protected function send_channel_packet($client_channel, $data)
    {
        ...
        while (\strlen($data)) {
            ...
            $temp = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($data, $max_size);
            $packet = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNs', NET_SSH2_MSG_CHANNEL_DATA, $this->server_channels[$client_channel], $temp);
    ...
    protected function disconnect_helper($reason)
    {
        ...
        if ($this->isConnected()) {
            $data = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::packSSH2('CNss', NET_SSH2_MSG_DISCONNECT, $reason, '', '');
            try {
                $this->send_binary_packet($data);
            } catch (\Exception $e) {
            }
    ...
    protected function format_log(array $message_log, array $message_number_log)
    {
        ...
        for ($i = 0; $i < \count($message_log); $i++) {
            ...
            do {
                ...
                $fragment = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($current_log, $this->log_short_width);
    ...
    public static function getSupportedEncryptionAlgorithms()
    {
        ...
        foreach ($engines as $engine) {
            foreach ($algos as $algo) {
                ...
                if ($obj instanceof \Google\Site_Kit_Dependencies\phpseclib3\Crypt\Rijndael) {
    ...
    public function setPreferredAlgorithms(array $methods)
    {
        ...
        foreach ($keys as $key) {
            ...
            if (\count($p) != \count($m)) {
                ...
                throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\UnsupportedAlgorithmException(\implode(', ', $diff) . $msg);
            }
        }
    ...
    public function getServerPublicHostKey()
    {
        ...
        switch ($this->signature_format) {
            ...
            case 'ecdsa-sha2-nistp521':
                $key = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\EC::loadFormat('OpenSSH', $server_public_host_key)->withSignatureFormat('SSH2');
                switch ($this->signature_format) {
                    case 'ssh-ed25519':
                        $hash = 'sha512';
                        break;
                ...
                break;
            case 'ssh-dss':
                $key = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\DSA::loadFormat('OpenSSH', $server_public_host_key)->withSignatureFormat('SSH2')->withHash('sha1');
                break;
            case 'ssh-rsa':
            case 'rsa-sha2-256':
            case 'rsa-sha2-512':
                ...
                list(, $signature) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $signature);
                $key = \Google\Site_Kit_Dependencies\phpseclib3\Crypt\RSA::loadFormat('OpenSSH', $server_public_host_key)->withPadding(\Google\Site_Kit_Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
                switch ($this->signature_format) {
                    case 'rsa-sha2-512':
                        $hash = 'sha512';
                        break;
            ...
            default:
                $this->disconnect_helper(NET_SSH2_DISCONNECT_HOST_KEY_NOT_VERIFIABLE);
                throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\NoSupportedAlgorithmsException('Unsupported signature format');
        }
    ...
    {
        if (\defined('Google\\Site_Kit_Dependencies\\NET_SSH2_LOGGING') && NET_SSH2_LOGGING == self::LOG_COMPLEX) {
