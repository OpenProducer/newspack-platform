...
abstract class OpenSSH
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (\strpos($key, 'BEGIN OPENSSH PRIVATE KEY') !== \false) {
            ...
            $magic = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::shift($key, 15);
            if ($magic != "openssh-key-v1\x00") {
                throw new \RuntimeException('Expected openssh-key-v1');
            }
            list($ciphername, $kdfname, $kdfoptions, $numKeys) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('sssN', $key);
            if ($numKeys != 1) {
            ...
            switch ($ciphername) {
                ...
                case 'aes256-ctr':
                    ...
                    list($salt, $rounds) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('sN', $kdfoptions);
            ...
            list($publicKey, $paddedKey) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $key);
            list($type) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $publicKey);
            if (isset($crypto)) {
                $paddedKey = $crypto->decrypt($paddedKey);
            }
            list($checkint1, $checkint2) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $paddedKey);
            // any leftover bytes in $paddedKey are for padding? but they should be sequential bytes. eg. 1, 2, 3, etc.
            if ($checkint1 != $checkint2) {
                if (isset($crypto)) {
                    throw new \Google\Site_Kit_Dependencies\phpseclib3\Exception\BadDecryptionException('Unable to decrypt key - please verify the password you are using');
                }
        ...
        list($type) = \Google\Site_Kit_Dependencies\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $key);
        self::checkType($type);
