...
use Google\Site_Kit_Dependencies\GuzzleHttp\Promise;
...
class Client implements \Google\Site_Kit_Dependencies\GuzzleHttp\ClientInterface
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\uri_for($config['base_uri']);
        }
    ...
    private function buildUri($uri, array $config)
    {
        ...
        $uri = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\uri_for($uri === null ? '' : $uri);
        if (isset($config['base_uri'])) {
            $uri = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\UriResolver::resolve(\Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\uri_for($config['base_uri']), $uri);
        }
    ...
    {
        $defaults = ['allow_redirects' => \Google\Site_Kit_Dependencies\GuzzleHttp\RedirectMiddleware::$defaultSettings, 'http_errors' => \true, 'decode_content' => \true, 'verify' => \true, 'cookies' => \false, 'idn_conversion' => \true];
     ...
     * @return Promise\PromiseInterface
     */
    private function transfer(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, array $options)
    {
        // save_to -> sink
        ...
        try {
            return \Google\Site_Kit_Dependencies\GuzzleHttp\Promise\promise_for($handler($request, $options));
        } catch (\Exception $e) {
     ...
     */
    private function applyOptions(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, array &$options)
    {
        $modify = ['set_headers' => []];
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
        ...
        if (isset($options['json'])) {
            $options['body'] = \Google\Site_Kit_Dependencies\GuzzleHttp\json_encode($options['json']);
            unset($options['json']);
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== \true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\_caseless_remove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\stream_for($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\_caseless_remove(['Authorization'], $modify['set_headers']);
        ...
        $request = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\modify_request($request, $modify);
