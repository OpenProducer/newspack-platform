...
final class Middleware
{
    ...
    public static function httpErrors()
    {
        return function (callable $handler) {
            return function ($request, array $options) use($handler) {
                ...
                return $handler($request, $options)->then(function (\Google\Site_Kit_Dependencies\Psr\Http\Message\ResponseInterface $response) use($request) {
                    $code = $response->getStatusCode();
                    if ($code < 400) {
                        return $response;
                    }
                    throw \Google\Site_Kit_Dependencies\GuzzleHttp\Exception\RequestException::create($request, $response);
                });
            };
        };
    }
     ...
     * @return callable Returns a function that accepts the next handler.
     */
    public static function log(\Google\Site_Kit_Dependencies\Psr\Log\LoggerInterface $logger, \Google\Site_Kit_Dependencies\GuzzleHttp\MessageFormatter $formatter, $logLevel = 'info')
    {
