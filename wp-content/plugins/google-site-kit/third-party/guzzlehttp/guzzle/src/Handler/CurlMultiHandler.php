...
class CurlMultiHandler
{
    ...
    {
        $this->factory = $options['handle_factory'] ?? new \Google\Site_Kit_Dependencies\GuzzleHttp\Handler\CurlFactory(50);
        if (isset($options['select_timeout'])) {
            $this->selectTimeout = $options['select_timeout'];
        } elseif ($selectTimeout = \Google\Site_Kit_Dependencies\GuzzleHttp\Utils::getenv('GUZZLE_CURL_SELECT_TIMEOUT')) {
    ...
    public function __invoke(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, array $options) : \Google\Site_Kit_Dependencies\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $id = (int) $easy->handle;
        $promise = new \Google\Site_Kit_Dependencies\GuzzleHttp\Promise\Promise([$this, 'execute'], function () use($id) {
    ...
    public function tick() : void
    {
        ...
        if ($this->delays) {
            $currentTime = \Google\Site_Kit_Dependencies\GuzzleHttp\Utils::currentTime();
        ...
        \Google\Site_Kit_Dependencies\GuzzleHttp\Promise\Utils::queue()->add(\Closure::fromCallable([$this, 'tickInQueue']));
    ...
    private function addRequest(array $entry) : void
    {
        ...
        if (empty($easy->options['delay'])) {
            ...
            $this->delays[$id] = \Google\Site_Kit_Dependencies\GuzzleHttp\Utils::currentTime() + $easy->options['delay'] / 1000;
        }
    }
    /**
    ...
    private function processMessages() : void
    {
        while ($done = \curl_multi_info_read($this->_mh)) {
            ...
            $entry['deferred']->resolve(\Google\Site_Kit_Dependencies\GuzzleHttp\Handler\CurlFactory::finish($this, $entry['easy'], $this->factory));
        }
    }
