...
class RedirectMiddleware
{
     ...
     * @return PromiseInterface
     */
    private function withTracking(\Google\Site_Kit_Dependencies\GuzzleHttp\Promise\PromiseInterface $promise, $uri, $statusCode)
    {
        return $promise->then(function (\Google\Site_Kit_Dependencies\Psr\Http\Message\ResponseInterface $response) use($uri, $statusCode) {
     ...
     */
    private function guardMax(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, array &$options)
    {
        $current = isset($options['__redirect_count']) ? $options['__redirect_count'] : 0;
        $options['__redirect_count'] = $current + 1;
        $max = $options['allow_redirects']['max'];
        if ($options['__redirect_count'] > $max) {
            throw new \Google\Site_Kit_Dependencies\GuzzleHttp\Exception\TooManyRedirectsException("Will not follow more than {$max} redirects", $request);
        }
    }
    /**
     * @param RequestInterface  $request
     ...
     */
    public function modifyRequest(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, array $options, \Google\Site_Kit_Dependencies\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        $modify['uri'] = $uri;
        \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\rewind_body($request);
        ...
        return \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\modify_request($request, $modify);
    }
    /**
     ...
     * @return UriInterface
     */
    private static function redirectUri(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, \Google\Site_Kit_Dependencies\Psr\Http\Message\ResponseInterface $response, array $protocols)
    {
