 ...
 * @final
 */
class MessageFormatter implements \Google\Site_Kit_Dependencies\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, ?\Google\Site_Kit_Dependencies\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
                ...
                case 'req_body':
                    ...
                    break;
                case 'res_body':
                    if (!$response instanceof \Google\Site_Kit_Dependencies\Psr\Http\Message\ResponseInterface) {
                        $result = 'NULL';
                        break;
                    }
     ...
     */
    private function headers(\Google\Site_Kit_Dependencies\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
