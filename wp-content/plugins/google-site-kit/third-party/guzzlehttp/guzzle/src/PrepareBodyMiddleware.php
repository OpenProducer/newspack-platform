...
class PrepareBodyMiddleware
{
    ...
    public function __invoke(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\Google\Site_Kit_Dependencies\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    /**
     * Add expect header
     *
     * @return void
     */
    private function addExpectHeader(\Google\Site_Kit_Dependencies\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
